...
def completion_rows(features: list[FeatureRow], surfaces: list[SurfaceRow]) -> list[str]:
...
def run(root: Path, completion: bool) -> tuple[list[str], list[str]]:
    failures, notices = [], []
    feature_path, surface_path = root / INVENTORY, root / SURFACES
    ...
    if completion: failures.extend(completion_rows(features, surfaces))
