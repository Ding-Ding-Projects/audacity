...
| `M3NumberField` | `currentValue`, `minValue`, `maxValue`, `step`, `decimals`, `measureUnitsSymbol`, `wrap`, `label`, `hint`, `readOnly`, `navigation`, `valueEdited(newValue)`, `valueEditingFinished(newValue)` | `IncrementalPropertyControl`. Built from `M3TextField` plus a decrement and increment `M3IconButton` pair, keeping the same public API so call sites swap mechanically. Promoted from a wrapper that had been duplicated locally three times (`src/preferences`, `src/effects`, `src/importexport`) into the shared library. |
