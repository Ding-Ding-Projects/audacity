...
int main(int argc, char** argv) {
    ...
    const std::vector<std::pair<const char*, std::function<void()>>> tests {
        ...
        {"shared real process deadline terminates child", [] {Fixture f;PdfProcessor::testTimeoutMilliseconds=100;bool started=false;PdfProcessor::testHook=[&](auto phase,const QString&){if(phase==PdfProcessor::TestPhase::ProcessStarted){started=true;QThread::msleep(120);}};QElapsedTimer timer;timer.start();auto result=f.run(f.request(PdfOperation::Inspect));require(started&&!result.ok&&result.message.contains("deadline")&&timer.elapsed()<5000,"bounded real child deadline");f.preserved();}},
        ...
        {"process allocation limit fails closed at launch", [] {Fixture f;PdfProcessor::testProcessMemoryBytes=1;QElapsedTimer timer;timer.start();auto result=f.run(f.request(PdfOperation::Inspect));require(!result.ok&&!result.cancelled&&timer.elapsed()<5000,"real child memory limit");f.preserved();}},
