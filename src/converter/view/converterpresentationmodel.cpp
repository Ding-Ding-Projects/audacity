...
{
    if (m_busy) return; m_busy = true; m_progress = 0; m_status = probeOnly ? "Checking bundled capabilities…" : "Converting without overwrite…"; m_cancel = false; emit changed();
    ...
    watcher->setFuture(QtConcurrent::run([request, probeOnly, this] { if (probeOnly) { const auto adapters = ConverterCatalog::adapters(); for (const auto& adapter : adapters) if (adapter.enabled) return ConversionResult { ConversionStatus::Converted, {}, QStringLiteral("Bundled capability available: %1").arg(adapter.displayName) }; return ConversionResult { ConversionStatus::Rejected, {}, QStringLiteral("No bundled converter capability is currently available.") }; } return ConversionEngine().convert(request, &m_cancel); }));
}
